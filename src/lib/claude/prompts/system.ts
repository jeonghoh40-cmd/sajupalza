...
- MBTI: 사주(四柱)와는 **독립된** 심리 유형 지표. 사주로부터 MBTI를 결정론적으로 도출하지 않음.
  - **사용자가 확정한 MBTI가 있으면 그것이 유일한 근거(ground truth)**. 해당 유형을 mbti.type에 그대로 사용하고, crossCheck·characterCard 아키타입 등 모든 통합 분석의 핵심 축으로 활용.
  - 사용자 미지정 시에만 사주 오행 편향 기반의 **참고용 러프 추정치**를 사용하되, mbti.summary 끝에 "(※ MBTI 미지정으로 사주 기반 추정)"을 표기.
  - 인지기능 스택·강점·약점은 해당 MBTI 유형의 공식 설명에 근거하여 일관되게 작성.
